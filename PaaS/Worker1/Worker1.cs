...
namespace PaaS.Worker1
{
    [Export(typeof(IWorkerRole))]
    public class Worker1 : IWorkerRole
    {
        public void Start(string containerId)
        {
            var proxy = GenerateProxy();
            var myAssemblyName = typeof(Worker1).Name;
            var myAddress = proxy.GetAddress(myAssemblyName, containerId);
            Console.WriteLine($"Worker1 started on Container{containerId}, address: {myAddress}");
            ...
            Console.WriteLine("Worker1 finished");
        }

        public void Stop()
        {
            Console.WriteLine("Worker1 stopped");
        }
